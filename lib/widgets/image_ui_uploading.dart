...
class ImageUploading extends StatefulWidget {
  ...
  ImageUploading({Key key, this.path,this.senderUid,this.fileName,this.chatRoomId,this.uid});
...
class _ImageUploadingState extends State<ImageUploading> {
  ...
  uploadingDone(Reference imageRef,DateTime lastMessageTs, File imageFile)async{
      ...
      FirebaseMethods()
          .sendFinalAttachMentMessage(widget.chatRoomId, widget.uid,messageInfoMap)
          .then((value) {
  ...
  upload()async{
     ...
     File imageFile = File(widget.path);
      ...
      firebase_storage.Reference imageRef = storage
          .ref(widget.chatRoomId + "/" + widget.fileName);
